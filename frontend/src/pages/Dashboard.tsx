...
const Dashboard: React.FC = () => {
  ...
  const { data: history = [], isLoading: historyLoading } = useCarbonHistoryQuery();
  const { data: recs = [], isLoading: recsLoading } = useRecommendationsQuery();

  const loading = summaryLoading || historyLoading || recsLoading;

  // Sync user score/points context with dashboard updates
  useEffect(() => {
    if (summary) {
      refreshUser();
    }
  ...
  const pieData = chronologicalHistory.length > 0 ? [
    { name: t('transportation'), value: chronologicalHistory[chronologicalHistory.length - 1].transportation?.emission_co2 || 0, color: '#06B6D4' },
    { name: t('energy'), value: chronologicalHistory[chronologicalHistory.length - 1].energy?.emission_co2 || 0, color: '#F59E0B' },
    { name: t('food'), value: chronologicalHistory[chronologicalHistory.length - 1].food?.emission_co2 || 0, color: '#10B981' },
    { name: t('lifestyle'), value: chronologicalHistory[chronologicalHistory.length - 1].lifestyle?.emission_co2 || 0, color: '#8B5CF6' }
  ].filter(item => item.value > 0) : [];

  // Format historical bar chart data
  const barData = chronologicalHistory.map(item => ({
    name: item.date,
  ...
  if (loading) {
    return <LoadingSpinner message="Loading Eco Dashboard..." className="min-h-[70vh]" />;
  }
    ...
    <div className="flex flex-col gap-6 text-left">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <button 
            ...
            className="flex items-center gap-2 px-4.5 py-2.5 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 hover:bg-slate-50 dark:hover:bg-slate-850 text-xs font-bold text-slate-700 dark:text-slate-250 transition-colors shadow-sm cursor-pointer"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="glass-panel p-6 rounded-3xl flex items-center justify-between shadow-sm">
            <div className="flex flex-col gap-1.5">
              ...
              <span className="text-2xs font-semibold text-slate-455 dark:text-slate-400">CO2 Equivalent</span>
            </div>
          ...
          <div className="glass-panel p-6 rounded-3xl flex items-center justify-between shadow-sm">
            <div className="flex flex-col gap-1.5">
              ...
              <span className="text-2xs font-semibold text-slate-455 dark:text-slate-400">Calculated Index</span>
            </div>
      ...
      {chronologicalHistory.length === 0 ? (
        <div className="glass-panel p-12 rounded-3xl text-center flex flex-col items-center justify-center gap-4 mt-2">
          ...
          <p className="text-sm text-slate-550 dark:text-slate-400 max-w-md leading-relaxed">
            Begin by calculating your carbon footprint using our Multi-step questionnaire to unlock dashboard analytics and insights.
          </p>
          <Link to="/calculator" className="eco-gradient text-white text-sm font-bold px-6 py-3 rounded-xl mt-2 shadow-lg shadow-eco-600/15">
            Run Calculator Now
          </Link>
        </div>
      ) : (
        <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <div className="glass-panel p-6 rounded-3xl lg:col-span-5 shadow-sm flex flex-col justify-between">
                  ...
                  summary.badges.map((badge, idx) => (
                    <div 
                      ...
                      className={`flex items-center justify-between p-3 rounded-2xl border text-xs font-bold ${
                        BADGE_COLORS[badge] || 'bg-slate-50 text-slate-700 border-slate-200'
                      }`}
                    >
              ...
              <Link to="/challenges" className="flex items-center justify-between text-xs font-bold text-eco-600 hover:text-eco-700 dark:text-eco-400 dark:hover:text-eco-350 border-t border-slate-100 dark:border-slate-800 pt-3">
            ...
            {/* AI Advisor Card */}
            <div className="glass-panel p-6 rounded-3xl lg:col-span-7 shadow-sm flex flex-col justify-between font-semibold">
              <div>
                <h3 className="font-bold text-slate-900 dark:text-white text-base flex items-center gap-2">
              ...
              <div className="flex flex-col gap-3.5 my-4">
                {recs.length === 0 ? (
                  <p className="text-xs text-slate-450 text-center py-6">No recommendations available. Run calculator first!</p>
                ) : (
                    ...
                    <div key={idx} className="bg-slate-50/50 dark:bg-slate-850/50 border border-slate-100 dark:border-slate-800/80 rounded-2xl p-4 text-left">
                      <div className="flex items-center justify-between mb-1.5">
                        ...
                        <span className={`text-2xs font-extrabold px-2 py-0.5 rounded-md uppercase ${
                          rec.impact === 'High' ? 'bg-rose-50 text-rose-600 dark:bg-rose-950/25 dark:text-rose-400' : 'bg-slate-100 text-slate-650 dark:bg-slate-800 dark:text-slate-400'
                        }`}>
                          {rec.impact} Impact
                        </span>
                      </div>
                      <h4 className="font-bold text-slate-900 dark:text-white text-sm">{rec.title}</h4>
                      <p className="text-xs text-slate-550 dark:text-slate-400 mt-1 leading-normal font-medium">{rec.tip}</p>
